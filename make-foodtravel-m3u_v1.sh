...
GetVideos(){
  ...
  echo "$(date)|INFO|Pull 500 random videos from work file and add to list file" >> "$LOGFILE"
  awk 'BEGIN{srand(); }
  { a[NR]=$0 }
  END{
    for(i=1; i<=500; i++){
...
if [ -f "$LOCKFILE" ]; then
  ...
  # create single list of all videos in specific folders (array not supported)
  GetVideos "/storage/nas/media/Other/Food and Travel"
  ...
  echo "$(date)|INFO|Pull 250 random videos from the list into a new file" >> "$LOGFILE"
  awk 'BEGIN{srand(); }
  { a[NR]=$0 }
  END{
    for(i=1; i<=250; i++){
