...
if [ -f "$LOCKFILE" ]; then
  ...
  if [ $LINES -gt 5 ]; then
    ...
    TELEGRAM_MESSAGE="XBMC-2411: Error creating M3U playlist. Lock file exists."
...
if [ ! -f "$VIDEO_LIST" ]; then
  ...
  if [ $LINES -gt 1 ]; then
    ...
    echo $(date)"|INFO|Sending Telegram alert. Exiting." >> $LOGFILE
    TELEGRAM_MESSAGE="XBMC-2411: Error creating M3U playlist. Source files missing."
    curl -s -X POST https://api.telegram.org/bot$TELEGRAM_TOKEN/sendMessage -d chat_id=$TELEGRAM_CHAT_ID -d text="$TELEGRAM_MESSAGE" > /dev/null
