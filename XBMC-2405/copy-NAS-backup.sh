...
# plugin data, and the flash drive. The backup for NAS4 is currently scheduled
# to run on Saturdays at 3:05 AM.
#
...
# --  --     --   --   --        -----------------------------------------------
# 30   0      *    *   Sunday    /storage/scripts/copy-NAS-backup.sh
#
#-------------------------------------------------------------------------------
...
if [ $DAYOFWEEK -eq 7 ]; then
  ...
  DIR=$(ssh root@haven-nas4.local ls -tr1 /mnt/user/Backups/Unraid/NAS4/ | tail -1)

  # Copy file from remote host using variable
  scp -r root@haven-nas4.local:/mnt/user/Backups/Unraid/NAS4/$DIR/ /storage/backup/
